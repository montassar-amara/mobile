...
@Component({
  ...
  templateUrl: './edit-report-widget.component.html',
  styleUrls: ['./edit-report-widget.component.scss']
})
export class EditReportWidgetComponent implements OnInit, OnDestroy {
  isFreeCompany = false;
  analysts:IUser[] = []
  ...
  getReport(report_id: string) {
    this.companyService.getOneReport(report_id).subscribe(res => {
  ...
  onUpdate(): void {
    ...
    this.companyService.updateTempReport(payload).subscribe((res) => {
      this.location.back()
    })
  }
